...
def schedule_evs(num_days, num_periods_day, peak_periods, off_peak_periods,
    ...
    off_peak_periods = {i + 1 for i in off_peak_periods}
    if network_tariff_peak == network_tariff_off_peak:
        peak_periods = {i + 1 for i in range(num_periods_day)}
        network_tariff_off_peak = 0
    else:
        # minizinc index starts from 1
        peak_periods = {i + 1 for i in peak_periods}
    ...
    ins["PEAK_PERIODS"] = peak_periods
    ins["OFF_PEAK_PERIODS"] = off_peak_periods
    ins["num_evs"] = num_evs
